...
- Day1用10個のprompt packetは `outputs/day1/image_generation/prompt_packets/day1_10_image_prompts_for_gpt/` に集約済み。
- Page1 / Page2の既存方向性画像がある場合は、repoへcandidateとして配置する。
- Page1 / Page2の既存画像を配置できない場合は、Page1から再生成する。
- Page1 / Page2候補が配置済みなら、次はPage3 candidateを1枚だけ生成する。
- Page4 candidateを1枚だけ生成する。
- Page5 candidateを1枚だけ生成する。
- Page6 candidateを1枚だけ生成する。
- 必要ならYuu / Sora / Miho referenceを生成する。
- 必要ならcommon UI partsを生成する。これは後続のカード、ワークシート制作にも使える。
...
- 親向け画像・図解用10個のprompt packetは `outputs/parents/image_generation/prompt_packets/` に集約済み。
